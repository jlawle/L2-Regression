...
def calculate_loss_r(n, d, w, l, y, x):
    # Sum predictions
    sum1 = 0
    for i in range(n):
        f_x = predict(w, x[i])
        sum1 += np.square(f_x - y[i])
    ...
    l_const = l/(2*d)
    n_const = 1/(2*n)
    return n_const*sum1 + l_const*sum2

# pd_loss(48, 16, weights, lambda_r, ynorms, norms, feature # of concern (jth feature))
def pd_loss(n, d, w, l, y, x, j):
    sum1 = 0
    for i in range(n):
        f_x = predict(w, x[i])
        sum1 += (f_x - y[i])*x[i][j]
    pdloss = (1/n)*sum1 + l/(2*(d+1))*w[j]
    return pdloss
...
while not_converged:
    ...
    for j in range(len(weights)):
        #determine pd of wj
        new_weight = weights[j] - (alpha * pd_loss(48, 16, weights, lambda_r,  train_norms_y, train_norms, j))
        new_weights.append(new_weight)

    # Get loss
    Jk = calculate_loss_r(48, 16, weights, lambda_r, train_norms_y, train_norms)
    Jkw.append(Jk)
...
plt.legend()
plt.title("Gradient Descent with Ridge Regularization")
...
# plot the MSE agaisst the test (20%) data
